    ...
    for epoch in range(round(args.epochs / args.eval_every_n_epochs)):
        ...
        if not isinstance(clf, StaticQK):
            try:
                ...
                pass
        # ── Early stopping + best checkpoint saving (patched) ──────────────
        if len(clf.hist.get('loss_valid', [])) > 0:
            import torch, shutil
            cur_val = clf.hist['loss_valid'][-1]
            if not hasattr(clf, '_best_val_loss'):
                clf._best_val_loss = float('inf')
                clf._no_improve    = 0
            if cur_val < clf._best_val_loss:
                clf._best_val_loss = cur_val
                clf._no_improve    = 0
                best_path = f'./data/model/mhn_best_checkpoint_{fn_postfix}.pt'
                torch.save(clf.state_dict(), best_path)
                print(f'   [ES] New best val_loss={cur_val:.4f} -> saved {best_path}')
            else:
                clf._no_improve += 1
                print(f'   [ES] No improvement {clf._no_improve}/10 (best={clf._best_val_loss:.4f})')
                if clf._no_improve >= 10:
                    print(f'   [ES] Early stopping triggered! Best val_loss={clf._best_val_loss:.4f}')
                    break
        # ───────────────────────────────────────────────────────────────────
