"""
Mondrian (per-tier) conformal selective prediction for LocalRetro.
Inputs:  /workspace/LocalRetro/outputs/localretro_baseline.csv
         /workspace/LocalRetro/outputs/ood_frequency_analysis.csv
         /workspace/LocalRetro/outputs/LocalRetro_USPTO_50K_mcdropout_uq.txt
Outputs: prints global vs Mondrian comparison table
         saves chart to /workspace/LocalRetro/outputs/conformal_solution.png
"""
import numpy as np
import pandas as pd
import matplotlib
...
# ── Load data ─────────────────────────────────────────────────────────────────
baseline = pd.read_csv('/workspace/LocalRetro/outputs/localretro_baseline.csv')
ood      = pd.read_csv('/workspace/LocalRetro/outputs/ood_frequency_analysis.csv')
mc       = pd.read_csv('/workspace/LocalRetro/outputs/LocalRetro_USPTO_50K_mcdropout_uq.txt', sep='\t')

df = baseline.merge(ood[['mol_idx','bucket','frequency']], on='mol_idx')
df = df.merge(mc[['mol_idx','std_top_score']], on='mol_idx')
df['uq'] = 1 - df['top_score']
df['tier'] = pd.Categorical(df['bucket'], categories=sorted(df['bucket'].unique()), ordered=True)

# ── Logistic combiner (softmax + MC std) ─────────────────────────────────────
...
# ── Split into calibration / test halves ─────────────────────────────────────
...
for t in df['tier'].cat.categories:
    ...
    if cm.sum() < 20 or tm.sum()==0: continue
...
ax.set_title('Global (marginal) threshold\nBreaks guarantee in rare tiers', fontsize=11)
ax.legend(fontsize=9)
ax.text(0, 104, '100% error!', ha='center', fontsize=10, color='#C0392B', fontweight='bold')

ax = axes[1]
m_errs = [r['err_kept']*100 if r['err_kept'] else 0 for _,r in mond.iterrows()]
m_covs = [r['coverage'] for _,r in mond.iterrows()]
bars = ax.bar(range(4), m_errs, color=colors, alpha=0.85, width=0.6)
ax.axhline(TARGET*100, color='black', linestyle='--', linewidth=1.5, label=f'Target ({TARGET*100:.0f}%)')
...
ax.set_title('Mondrian (per-tier) threshold\nAbstains in rare tier; holds target elsewhere', fontsize=11)
ax.legend(fontsize=9)

plt.suptitle('Selective Prediction: Global vs. Mondrian Conformal (target error = 20%)',
             fontsize=12, y=1.01)
plt.tight_layout()
plt.savefig('/workspace/LocalRetro/outputs/conformal_solution.png', bbox_inches='tight', dpi=150)
print("\nFigure saved to /workspace/LocalRetro/outputs/conformal_solution.png")
